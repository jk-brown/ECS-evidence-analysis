...
eoc_warming_results <- lapply(names(weighted_model_results), function(df_name){
  ...
  # get data frame corresponding with the current element by name
  df <- weighted_model_results[[df_name]]
  
  # get weights data corresponding with the current element by name
  weights_df <- model_weights[[df_name]]
  ...
  # add scenario column to metric_df 
  metric_df$scenario <- df_name
  
  #merge weights with metric_df based on run_number
  metric_df <- left_join(metric_df, weights_df, by = "run_number")
  
  return(metric_df)
})

# bind list eoc_warming_results to a data frame
eoc_warming_df <- do.call(rbind, eoc_warming_results)

# save the result for future 

```

The result is a list of data frames, one for each ECS configuration, with the model run_number, metric values, name of the scenario, and associated weight. 

In the final step we convert the list of a data frame that can be used for future visualization.
